...
public class ReplyController {
	...
	@ResponseBody
	public ResultVO updateReply(ReplyVO inVO, HttpSession session) throws SQLException {
	...
	@ResponseBody
	public ResultVO deleteReply(ReplyVO inVO, HttpSession session)  throws SQLException {
		ReplyVO outVO = service.viewReplyDetail(inVO);
		if(!sessionService.isSessionMatched(session, outVO.getMemberId())) {
			String errorMessage = messageSource.getMessage("login.permission.error", null, Locale.getDefault());
	        return new ResultVO("0", errorMessage);
	...
	@ResponseBody
	public ResultVO saveReply(ReplyVO inVO, HttpSession session) throws SQLException {
